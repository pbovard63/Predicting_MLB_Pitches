'''
This file contains functions that utilize classification and regression functions in conjunction to predict pitch type and location of an MLB pitch.  Utilizes pitch type counts in the last 5 pitches the pitcher has thrown as classification features.
'''
...
def split_pitch_data_l5(df, test_size=.2, seed=32):
...
def dataframe_setup_l5(player_name, data, split_size = 0.2):
    '''
    ...
    training_pitches, val_pitches = split_pitch_data_l5(output_df, test_size=split_size)
    return training_pitches, val_pitches

def random_forest_pitch_classification_l5(train_df, val_df):
    '''
...
def xgboost_pitch_classification_l5(train_df, val_df):
    '''
    Arguments: takes in a training and validation dataframe of a pitcher.
...
def px_linear_regression_l5(train_df, val_df):
    '''
...
def pz_linear_regression_l5(train_df, val_df):
    '''
    Arguments: takes in a training and validation dataframe.
...
def px_rf_regression(train_df, val_df):
    '''
    Arguments: takes in a training and validation dataframe.
    Returns: a RandomForest regression model of the x location of the pitch (px), with the predicted values mapped onto the validation dataframe input.
    '''
    #Setting up the columns needed:
    px_cols = ['Cluster','inning', 'top', 'on_1b', 'on_2b', 'on_3b', 'b_count', 's_count', 'outs', 'stand_R',
       'pitcher_run_diff','last_pitch_speed', 'last_pitch_px', 'last_pitch_pz','pitch_num','cumulative_pitches',
    ...
    #Setting up the x and y inputs for RF regression:
    X_px_tr = train_df[px_cols]
    y_px_tr = train_df['px']
    X_px_val = val_df[px_cols_val]
    y_px_val = val_df['px']

    #Fitting RF regression on the training data, then predicting and scoring on the validation set:
...
def pz_rf_regression_l5(train_df, val_df):
    '''
    Arguments: takes in a training and validation dataframe.
    Returns: a RandomForest regression model of the z location of the pitch (pz), with the predicted values mapped onto the validation dataframe input.
    '''
    #Setting up the columns needed:
    pz_cols = ['Cluster','inning', 'top', 'on_1b', 'on_2b', 'on_3b', 'b_count', 's_count', 'outs', 'stand_R',
       'pitcher_run_diff','last_pitch_speed', 'last_pitch_px', 'last_pitch_pz','pitch_num','cumulative_pitches',
    ...
    #Fitting RF regression on the training data, then predicting and scoring on the validation set:
    rf = RandomForestRegressor(criterion='mae')
    rf.fit(X_pz_tr, y_pz_tr)
    pz_pred = rf.predict(X_pz_val)
    pz_r2 = rf.score(X_pz_val, y_pz_val)
    pz_mae = mae(y_pz_val, pz_pred)
    print('Val Pz R^2: {}'.format(pz_r2))
    print('Val Pz MAE: {} ft.'.format(pz_mae))
...
def pitch_prediction_modeling_pipeline_l5(player_name, data, split_size = 0.2, class_method = 'RandomForest', reg_method = 'Linear'):
    '''
    Arguments: takes in a player name and a dataframe of MLB league-wide pitch data.  The default split size for train/validation by at bat is 0.20.
    ...
    training_pitches, val_pitches = dataframe_setup_l5(player_name, data, split_size = 0.2)
    ...
    if class_method == 'RandomForest':
        ...
        rf_model, val_df = random_forest_pitch_classification_l5(training_pitches, val_pitches)
    
    elif class_method == 'XGBoost':
        xg_model, val_df = xgboost_pitch_classification_l5(training_pitches, val_pitches)
    
    if reg_method == 'Linear':
        print('Linear Regression:')
        #Running linear regression predictions on the pitch x coordinate (px):
        px_model, val_df = px_linear_regression_l5(training_pitches, val_df)
        
        #Running linear regression predictions on the pitch z coordinate (pz):
        pz_model, val_df = pz_linear_regression_l5(training_pitches, val_df)
    
    elif reg_method == 'RandomForest':
        print('Random Forest Regression:')
        #Running Random Forest regression predictions on the pitch x coordinate (px):
        px_model, val_df = px_rf_regression_l5(training_pitches, val_df)
        
        #Running Random Forest regression predictions on the pitch z coordinate (pz):
        pz_model, val_df = pz_rf_regression_l5(training_pitches, val_df)
    
    #returning the validation dataframe with the predictions mapped out:
    return val_df

def multiple_pitcher_predictions_l5(player_name_list, data, split_size = 0.2, class_method = 'RandomForest', reg_method = 'Linear'):
    '''
    ...
    for player in player_name_list:
        val_df = pitch_prediction_modeling_pipeline_l5(player, data, split_size = split_size, class_method=class_method, reg_method=reg_method)
