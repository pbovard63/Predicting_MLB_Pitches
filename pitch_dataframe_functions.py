'''
Functions to build out portions of the pitch data.
'''
import numpy as np
import pandas as pd
...
def pitcher_pitch_rate_gen(df, threshold=0.05):
    '''
    Arguements: takes in a dataframe of pitch by pitch data, and a minimum threshold for pitch rate, for a pitch to be included in the arsenal. Default threshold is 0.05.
    Returns: a dataframe with pitcher names as rows and columns of pitch type, with values being frequency thrown (if above the given threshold).
    '''
